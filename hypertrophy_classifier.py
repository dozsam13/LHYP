...
class HypertrophyClassifier(nn.Module):
    def __init__(self, c1c2, c2c3, c3c4, c4c5, c5c6, c6c7, c7l1):
        ...
        self.conv8 = nn.Conv2d(in_channels=c7l1, out_channels=40, kernel_size=(1, 1), stride=1)
        ...
        self.conv8_bn = nn.BatchNorm2d(40)
    ...
    def forward(self, x):
        ...
        temp = self.relu(self.conv1_bn(self.conv1(temp)))  # (108, 108)
        ...
        temp = self.max_avg_pool(temp)  # (3, 3)

        temp = self.relu(self.conv8_bn(self.conv8(temp)))
        temp = self.dropout_02(temp)

        temp = temp.view(-1, 9 * 40)
