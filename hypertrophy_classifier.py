...
class HypertrophyClassifier(nn.Module):
    def __init__(self, c1c2, c2c3, c3c4, c4c5, c5c6, c6c7, c7l1):
        ...
        self.conv8 = nn.Conv2d(in_channels=c7l1, out_channels=30, kernel_size=(1, 1), stride=1)
        ...
        self.linear1 = nn.Linear(25 * 30, 25)
        ...
        self.linear1_bn = nn.BatchNorm1d(25)
        self.linear2_bn = nn.BatchNorm1d(3)
    ...
    def forward(self, x):
        ...
        temp = self.maxpool_2_2(temp)  # (5, 5)
        temp = self.relu(self.conv7_bn(self.conv7(temp)))  # (5, 5)

        temp = self.relu(self.conv8_bn(self.conv8(temp)))

        temp = temp.view(-1, 25 * 30)
        temp = self.relu(self.linear1_bn(self.linear1(temp)))

        temp = self.dropout_02(temp)
        temp = self.linear2_bn(self.linear2(temp))
